...
A minimalist FastAPI micro-service that provisions SIP extensions into an **Asterisk 20 (LTS)** PBX using the **PJSIP Realtime** engine with a **SQLite3** backend.
...
The API manages three PJSIP Realtime tables (defined in `schema.sql`):
...
All three rows share the same `id` (the username/extension number), which is the convention Asterisk's Realtime engine expects.
...
### Remove a SIP Extension

```
DELETE /provision/{username}
```

Removes all three Realtime rows (auth, AOR, endpoint) for the given username. Asterisk will stop recognising the extension on its next Realtime lookup — no `pjsip reload` needed.
